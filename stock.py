...
st.sidebar.markdown("- Select/enter tickers (e.g., AAPL,GOOGL,TSLA).")
...
selected_tickers = st.sidebar.multiselect("Select stock tickers:", ticker_options, default=["AAPL", "GOOGL", "TSLA"])
tickers_input = st.sidebar.text_input("Enter custom tickers (comma-separated):", "")
# Process custom tickers and combine with selected tickers
...
for stock_symbol in tickers:
    ...
    try:
        ...
        if data.empty:
            st.warning(f"No data for {stock_symbol}. Try a valid ticker (e.g., AAPL).")
    ...
    try:
        ...
        # Display last 5 rows with formatted dates for display
        display_data = data[['Close', 'SMA20', 'RSI']].tail().copy()
        ...
        # Oversold days
        oversold_days = data[data['RSI'] < 30][['Close', 'RSI']].copy()
        ...
        # Download data
        csv = data[['Close', 'SMA20', 'RSI']].to_csv()
        ...
        fig1.add_trace(go.Scatter(x=data.index, y=data['SMA20'], mode='lines', name='20-Day SMA', line=dict(color='orange')))
        fig1.add_trace(go.Scatter(x=[data.index[-1]], y=[data['Close'].iloc[-1]], mode='markers', name='Latest Price', marker=dict(color='blue', size=10)))
        fig1.add_trace(go.Scatter(x=[data.index[-1]], y=[data['SMA20'].iloc[-1]], mode='markers', name='Latest SMA', marker=dict(color='orange', size=10)))
...
if oversold_stocks:
    ...
    st.header("🚨 Oversold Stocks (RSI < 30)")
    st.info("No stocks with RSI < 30. Try AMD, F, BA.")
