...
tickers = [ticker.strip().upper() for ticker in tickers_input.split(",") if ticker.strip()] + selected_tickers
...
for stock_symbol in tickers:
    ...
    try:
        ...
        st.dataframe(data[['Close', 'SMA20', 'RSI']].tail().style.format("{:.2f}").background_gradient(cmap="RdYlGn", subset=["RSI"]))
